...
signed main(){
    ...
    VEC(ll,a,n);
    vv(ll,cnt,2,n,0);
    rep(n)rep(j,i+1,n)cnt[0][i]+=a[i]>a[j],cnt[1][j]+=a[j]>a[i];
    ll kake=rtmod(k*(k+1)/2,MOD);
    ll kake2=rtmod(k*(k-1)/2,MOD);
    ll ans=rtmod(sum(cnt[0])*kake,MOD);
    ans+=rtmod(sum(cnt[1])*kake2,MOD);
    out(rtmod(ans,MOD));
}//time 0:42:53
