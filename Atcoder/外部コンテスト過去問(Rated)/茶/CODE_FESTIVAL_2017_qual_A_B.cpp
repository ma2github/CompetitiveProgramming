...
bool solve(ll n,ll m,ll k){
  ...
  rep(mn/2+1){
    ll init=i*mx;
    if((init>k)^(2*i>mn))continue;
    if(!(mn-2*i)){if(k==init)return true;else continue;}
    if(!rtmod(k-init,mn-2*i)&((k-init)/(mn-2*i)<=mx))return true;
  }
  return false;
}
signed main(){
    ...
    Yes(solve(n,m,k));
    /*
    rep(a,n+1)rep(b,m+1)if(m*n==k+a*b+(n-a)*(m-b))return Yes(1);
    Yes(0);
    */
}
