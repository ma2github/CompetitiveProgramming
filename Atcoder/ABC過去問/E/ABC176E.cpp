...
using std::map;
map<ll,ll>memoh;//<キー，その要素＞，キーの検索が早い，キーは昇順にソートされる
map<ll,ll>memow;
std::vector<pll> b;
signed main(){
    /*以下コード*/
    ...
    rep(m){
      ...
      memoh[hi]++;
      memow[wi]++;
      b.push_back(mp(hi,wi));
      chmax(maxh,memoh[hi]);
      chmax(maxw,memow[wi]);
    }
    ll hs=0;
    ll ws=0;
    each(i,memoh)if(i.second==maxh)hs++;
    each(i,memow)if(i.second==maxw)ws++;
    ll koho=hs*ws;
    if(koho<=m)each(i,b)if(memoh[i.first]==maxh&&memow[i.second]==maxw)koho--;
    out(maxh+maxw-!koho);
}
