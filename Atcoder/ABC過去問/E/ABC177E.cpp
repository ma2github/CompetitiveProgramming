...
signed main(){
    ...
    LL(n);
    VEC(ll,a,n);
    if(reduce(all(a),0,gcd<ll,ll>)!=1)return out("not coprime");
    else{
      std::vector<ll> d(1e6+1+1);
      rep(1e6+1)d[i]=i;
      for(ll i=2;i*i<=1e6;i++){
        if(d[i]!=i)continue;
        for(ll j=1;i*j<=1e6;j++){
          if(d[i*j]!=i*j)continue;
          d[i*j]=i;
        }
      }
      std::vector<bool>memo(1e6+1,false);
      rep(n){
        ll num=a[i];
        while(num>1){
          if(memo[d[num]])break;
          memo[d[num]]=true;
          ll p=num/d[num];
          while(d[num]==d[p])p/=d[num];
          num=p;
        }
        if(num>1)return out("setwise coprime");
      }
