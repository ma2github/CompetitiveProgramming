...
signed main(){
    ...
    a.push_back(0);
    ll sm=0,cur=0;
    rep(n+1)sm+=abs(a[i]-cur),cur=a[i];
    cur=0;
    rep(n){
      if((a[i+1]<=cur and cur<=a[i]) or (a[i]<=cur and cur<=a[i+1]))out(sm-2*abs(a[i]-cur));
      elif((cur<=a[i+1] and a[i+1]<=a[i]) or (a[i]<=a[i+1] and a[i+1]<=cur))out(sm-2*abs(a[i]-a[i+1]));
      elif((cur<=a[i] and a[i]<=a[i+1]) or (a[i+1]<=a[i] and a[i]<=cur))out(sm);
      cur=a[i];
    }
}
