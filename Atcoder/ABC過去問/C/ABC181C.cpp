...
signed main(){
    ...
    rep(i,n)rep(j,i+1,n)rep(k,j+1,n){
      if((xy[i][0]==xy[j][0] and xy[j][0]==xy[k][0]) or (xy[i][1]==xy[j][1] and xy[j][1]==xy[k][1]))return Yes();
      bool f=false;
      rep(p,2)if((xy[i][p]==xy[j][p]) or (xy[j][p]==xy[k][p]) or (xy[k][p]==xy[i][p]))f=true;
      if(f)continue;
      ll xji,yki,xki,yji;
      xji=xy[j][0]-xy[i][0];
      yki=xy[k][1]-xy[i][1];
