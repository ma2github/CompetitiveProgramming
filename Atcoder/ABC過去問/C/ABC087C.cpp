...
vv(ll,a,2,110);
ll rec(ll i,ll j){
  if(i<0||j<0)return -LINF;
  if(~dp[i][j])return dp[i][j];
  if(!i&!j)return dp[i][j]=a[0][0];
  return dp[i][j]=max(rec(i-1,j),rec(i,j-1))+a[i][j];
}*/
signed main(){
    /*以下コード*/
    //Using Cumulative sum O(n);
    LL(n);
    vv(ll,a,2,n);
    vv(ll,sm,2,n+1);
    rep(i,2)rep(j,n){
    ...
    ll ans=0;
    rep(j,n)chmax(ans,sm[0][j+1]+sm[1][n]-sm[1][j]);
    out(ans);
    // Using DP O(n^2);
  /*  LL(n);
