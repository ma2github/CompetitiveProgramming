...
signed main(){
    ...
    ll n=stoi(s);
    ll sz=s.size();
    if(sz<3){out(0);return 0;}
    ll ans=0;
    std::vector<ll> res(sz+1,0);
    rep(sz){
      if(!res[i])res[i]=facctorialMethod(i);
      rep(a,1,i-1){
        if(!res[a])res[a]=facctorialMethod(a);
        rep(b,1,i-a){
          if(!res[b])res[b]=facctorialMethod(b);
          if(i-a-b>0){
            if(!res[i-a-b])res[i-b-a]=facctorialMethod(i-b-a);
            ans+=res[i]/res[a]/res[b]/res[i-a-b];
          }
        }
      }
    }
    rep(a,1,sz-1)rep(b,1,sz-a){
      string str;
      ll c=sz-b-a;
      rep(a)str+='3';
      rep(b)str+='5';
      ...
      do{
        ll num=0,cnt=1;
        rrep(sz)num+=(str[i]-'0')*cnt,cnt*=10;
        if(num>n)break;
        ans++;
