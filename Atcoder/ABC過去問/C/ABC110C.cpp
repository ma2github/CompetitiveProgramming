...
signed main(){
    ...
    string a,b;
    size_t sz=s.size();
    map<char,ll>smap,tmap;
    std::vector<ll> scnt,tcnt;
    rep(sz){
      if(!smap[s[i]])scnt.push_back(s[i]);
      smap[s[i]]++;
      ll index=0;
      while(scnt[index]!=s[i])index++;
      a+=to_string(index);
    }
    rep(sz){
      if(!tmap[t[i]])tcnt.push_back(t[i]);
      tmap[t[i]]++;
      ll index=0;
      while(tcnt[index]!=t[i])index++;
      b+=to_string(index);
    }
    Yes(a==b);
}
