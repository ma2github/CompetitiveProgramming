...
signed main(){
    /*以下コード*/
    LL(n);
    std::vector<vector<ll>> table(n,vector<ll>(n,2));
    ll ans=0;
    //LL(n);
    rep(i,n){
      LL(ai);
    ...
    std::vector<vector<ll>> table1(n,vector<ll>(n,2));
    rep(1<<n){
      table1=table;
      ll cnt=n;
      ll b=1;
        rep(x,n){
          if(!(i&(1<<x))){cnt--;continue;}
          rep(y,n){
            if(!(i&(1<<y))&&table1[x][y]==1){b=0;break;}
            else if((i&(1<<y))&&table1[x][y]==0){b=0;break;}
          }
          if(b==0)break;
        }
      if(b==1)chmax(ans,cnt);
    }
    out(ans);
}/*二次元の表を作らなくても，別に誰が誰をなんと言ったかが言えれば良かったのか・・・*/
