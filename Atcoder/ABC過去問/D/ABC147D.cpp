...
signed main(){
    ...
    rep(i,1,n+1)cin>>a[i];
    vec(Modint,sm,n+1,0);
    vec(Modint,sm1,n+1,0);
    vec(Modint,lost,n+1,0);
    vv(ll,b,n+1,64,0);
    vec(ll,cnt,64,0);
    rep(i,1,n+1){
      sm[i]=sm[i-1]+a[i]%MOD;
      sm1[i]=sm1[i-1]+((a[i])%MOD*(i-1))%MOD+sm[i-1];
      lost[i]=lost[i-1];
      rep(j,61)b[i][j]=(a[i]>>j)&1,cnt[j]+=b[i][j],lost[i]+=(cnt[j]-1)*b[i][j]*2_M .pow(j+1);
    }
    Modint ans=sm1[n]-lost[n];
    out(ans);
}
