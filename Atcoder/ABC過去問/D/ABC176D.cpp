...
const ll dx2[] = {0, 1, 0, -1, 1, -1, 1, -1};
const ll dy2[] = {1, 0, -1, 0, 1, 1, -1, -1};
vector<vector<pair<int, int>>> mazeconv(ll N,ll i,ll j,std::vector<string>&s){
  vector<vector<pair<int, int>>> edges(N);
  return edges;
}
int bfs01(ll h,ll w,ll ch,ll cw){
  ll N=h*w;
  ll s=ch*w+cw;
    // 隣接リスト。
...
void add(ll mul,ll n,ll x,ll y,vector<string> &s,deque<pll>&que,ll cost[1100][1100],ll c){
  rep(n)if(x>=mul*dx[i]&&y>=mul*dy[i])if(s[x+mul*dx[i]][y+mul*dy[i]]=='.'){chmin(cost[x+mul*dx[i]][y+mul*dy[i]],cost[x][y]+c);que.push_front(mp(x+mul*dx[i],y+mul*dy[i]));}
}
signed main(){
    /*以下コード*/
    LL(h,w);
    ...
    dh--;dw--;
    deque<pll> que;
    que.push_back(mp(ch,cw));
    while(!que.empty()){
      pll p=que.front();
      que.pop_front();
      ll x=p.first,y=p.second;
      add(1,4,x,y,s,que,cost,0);
      add(2,8,x,y,s,que,cost,1);
      add(1,8,x,y,s,que,cost,1);
    }
    if(cost[dh][dw]==1e9)out(-1);
    else out(cost[dh][dw]);
}
