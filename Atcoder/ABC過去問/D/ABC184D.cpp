...
ld ans=0;
void dfs(ld a,ld b,ld c,ld p,ll cnt){
  if(abs(max(a,max(b,c))-100.0)<1){ans+=(ld)cnt*p;return;}
  if(a>0)dfs(a+1.0,b,c,p*a/(a+b+c),cnt+1);
  if(b>0)dfs(a,b+1.0,c,p*b/(a+b+c),cnt+1);
  if(c>0)dfs(a,b,c+1.0,p*c/(a+b+c),cnt+1);
}
void cal(ld a,ld b,ld c,ld t1,ld t2){
  ld t0=a+b+c-t1-t2;
  rep(i,100-ll(t1)){
    rep(j,100-ll(t2)){
      ld cnt=100.0-t0+i+j;
      ld sm=a+b+c;
      ld per=1.0;
      rep(x,t0,99)per*=(ld)x,per/=sm,sm+=1.0;
      rep(y,i)per*=(ld)(y+1.0),per/=sm,sm+=1.0;
      rep(x,j)per*=(ld)(x+1.0),per/=sm,sm+=1.0;
      rep(x,99-t0)per=cnt-(ld)x-1.0,per/=(ld)x+1.0;
      per*=99.0;per/=sm;
      ans+=per*cnt;
    }
  }
}
vvv(ld,dp,101,101,101,-1.0);
ll a,b,c;
ld rec(ll x,ll y,ll z){
  if(x<a or y<b or z<c)return 0.0;
  if(x==a and y==b and z==c)return 1.0;
  if(dp[x][y][z]!=-1.0)return dp[x][y][z];
  ld res=0.0;
  if(x+y+z>1){
    if(max(y,z)<100)res+=rec(x-1,y,z)*(x-1.0)/(x+y+z-1.0);
    if(max(x,z)<100)res+=rec(x,y-1,z)*(y-1.0)/(x+y+z-1.0);
    if(max(x,y)<100)res+=rec(x,y,z-1)*(z-1.0)/(x+y+z-1.0);
  }
  return dp[x][y][z]=res;
}
signed solve(){//main
  ...
  */
  cin>>a>>b>>c;
  rep(y,b,100)rep(z,c,100)ans+=rec(100,y,z)*(100.0+y+z-a-b-c);//out(rec(100,y,z));
  rep(x,a,100)rep(z,c,100)ans+=rec(x,100,z)*(x+100.0+z-a-b-c);//out(rec(x,100,z));
  rep(x,a,100)rep(y,b,100)ans+=rec(x,y,100)*(x+y+100.0-a-b-c);//out(rec(x,y,100));
  out(ans);
