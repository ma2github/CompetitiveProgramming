...
signed main(){
    /*以下コード*/
    /*なるべく計算は関数に頼らない（演算子でできる限り頑張る）
    桁数をちゃんと考慮する*/
    /*方針：「(i,j)が条件を満たす」は以下と同値
    「（下からi桁目までの数字）と（下からj桁目までの数字）を2019で割ったあまりが同じ」
    （互いを引けばあまりが差し引かれて2019の倍数になる）
    2019で割ったあまりのパターンは2019通りあるので，各あまりに対して何通りあるかをカウント
    あとはそのあまりパターンの個数nから2つ選ぶということでnC2を各あまりに対して計算して
    足し合わせるのを繰り返せば答えがもとまる
    */
    ll ans=0;
    STR(s);
    size_t n=s.size();
    ...
    ll t=1;
    rep(n){
      dp[i+1]=(dp[i]+t*(int)(s[n-1-i]-'0'))%2019;//dp[i]：下からi桁目までを１０進数と見たときの2019で割った余り
      mod[dp[i+1]]++;//そのあまりに何パターンあるか
      t*=10;
      t%=2019;
    }
    each(i,mod)if(i>1)ans+=i*(i-1)/2;//そのあまりのパターン数から2個選ぶ
    out(ans);
}
