...
//int N; // 全体の要素数
//int Q;//試行回数
ll doubling(const ll N,const ll Q,vector<ll> &a,vector<ll> &c){//cin>>N>>Q;//標準入力から要素数と試行回数を受け取る場合
...
std::vector<std::vector<ll>> vl(LOG_Q + 1, std::vector<ll>(N));
std::vector<std::vector<ll>> mx(LOG_Q + 1, std::vector<ll>(N));
//ll a[N];//各要素の次の行き先
...
for (ll k = 0; k < LOG_Q; ++k){
    for (int i = 0; i < N; ++i){
        if (next[k][i] == -1) {
            ...
            vl[k+1][i]=vl[k][i]+vl[k][next[k][i]];
            mx[k+1][i]=max(mx[k][i],vl[k][i]+mx[k][next[k][i]]);
        }
    }
}
