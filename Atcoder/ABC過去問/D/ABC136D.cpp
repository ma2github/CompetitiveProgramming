...
//int N; // 全体の要素数
//int Q;//試行回数
std::vector<ll> doubling(const ll N,const ll Q,vector<ll> &a){//cin>>N>>Q;//標準入力から要素数と試行回数を受け取る場合
...
// ----ここまで準備----
/*
ll p=init;// p番目の要素の「Q個次の要素」を求めることを考えます
...
signed main(){
    ...
    size_t n=s.size();
    std::vector<ll> v(n);
    rep(n)v[i]=i+(s[i]=='R')*2-1;
    std::vector<ll> nx=doubling(n,1<<20,v);
    vec(ll,ans,n,0);
    each(i,nx)ans[i]++;
    out(ans);
}
