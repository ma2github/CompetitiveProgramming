...
signed main(){
    ...
    each(ai,a)c[ai]++;
    each(bi,b)d[bi]++;
    rep(n)c[i+1]+=c[i],d[i+1]+=d[i];
    ll rrotate=0;
    ll cntmax=-LINF;
    rep(n)chmax(cntmax,(d[i+1]-d[i])+(c[i+1]-c[i])),chmax(rrotate,c[i+1]-d[i]);
    if(cntmax>n)return Yes(0);
    Yes(1);
    rep(n)cout<<b[rtmod((i+n)-rrotate,n)],(i<n-1)?cout<<' ':cout<<endl;
}
