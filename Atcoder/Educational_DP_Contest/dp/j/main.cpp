...
vec(int,f,4,0);
vec(int,a,310);
vvv(ld,dp,310,310,310,-1.0000);
ld rec(int i,int j,int k){
  if(!i and !j and !k)return 0.0;
  if(dp[i][j][k]>-1.0000)return dp[i][j][k];
  ld res=0.0;
  ld ml=1.0/(1.0-(ld)(n-i-j-k)/(ld)n);
  res+=ml;
  if(i)res+=(ld)i/(ld)n*rec(i-1,j,k)*ml;
  if(j)res+=(ld)j/(ld)n*rec(i+1,j-1,k)*ml;
  if(k)res+=(ld)k/(ld)n*rec(i,j+1,k-1)*ml;
  return dp[i][j][k]=res;
}
signed solve(){//main
  /*
