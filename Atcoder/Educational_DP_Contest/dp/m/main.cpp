...
int n,k;
vec(int,a,110);
vv(Modint,dp,110,100010,-1);
vv(bool,flag,110,100010,false);
Modint rec(int i,int num){
  if(!num)return 1;
  if(!i)return num<=a[0]?1:0;
  if(flag[i][num])return dp[i][num];
  flag[i][num]=true;
  Modint res=0;
  if(a[i]<num){
    res+=rec(i,num-1);
    res-=rec(i-1,num-a[i]-1);
    res+=rec(i-1,num);
  }else{
    res+=rec(i,num-1);
    res+=rec(i-1,num);
