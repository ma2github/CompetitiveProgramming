...
vv(ll,dp,100010,3,-1);
ll rec(ll d,ll l,std::vector<std::vector<ll>> &abc){
  if(~dp[d][l])return dp[d][l];
  if(!d)return dp[d][l]=abc[d][l];
  ll res=0;
  rep(3)if(i!=l)chmax(res,rec(d-1,i,abc)+abc[d][l]);
  return dp[d][l]=res;
}
signed main(){
    /*以下コード*/
