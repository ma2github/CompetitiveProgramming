...
ll rec(ll las,ll lim,std::vector<std::vector<ll>> &wv){
  if(las<0)return 0;
  if(~dp[las][lim])return dp[las][lim];
  if(!lim)return dp[las][lim]=0;
  ll res=rec(las-1,lim,wv);
  if(lim-wv[las][0]>=0)chmax(res,rec(las-1,lim-wv[las][0],wv)+wv[las][1]);
  return dp[las][lim]=res;
}
signed main(){
    /*以下コード*/
    LL(n,w);
