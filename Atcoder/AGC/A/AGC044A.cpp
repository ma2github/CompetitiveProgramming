...
ll rec(ll x){
  if(dp[x])return dp[x];
  if(!x)return dp[x]=0;
  if(x==1)return dp[x]=d;
  ll res=LINF/d>=x?x*d:LINF;
  chmin(res,rec(x/5)+(x%5)*d+c);
  chmin(res,rec((x+4)/5)+((5-x%5)%5)*d+c);
  chmin(res,rec(x/3)+(x%3)*d+b);
  chmin(res,rec((x+2)/3)+((3-x%3)%3)*d+b);
  chmin(res,rec(x/2)+(x%2)*d+a);
  chmin(res,rec((x+1)/2)+((2-x%2)%2)*d+a);
  return dp[x]=res;
}
signed solve(){//main
  /*
