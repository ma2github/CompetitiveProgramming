...
signed main(){
    /*以下コード*/
    LL(n,k);
    std::vector<int> hu,zero,sei;
    rep(n){
      LL(ai);
      if(!ai)zero.push_back(ai);
      elif(ai<0)hu.push_back(ai);
      else sei.push_back(ai);
    }
    size_t NumHu=hu.size();
    size_t NumZero=zero.size();
    size_t NumSei=sei.size();
    ll calchu=NumHu*NumSei;
    ll calczero=NumZero*NumSei;
    ll calcsei=n*(n-1)/2-calchu-calczero;
    if(calchu>k){
    }
    if(k<=calchu+calczero)return out(0);
}
