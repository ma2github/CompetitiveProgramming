...
signed solve(){//main
  ...
  */
  ...
  --x,--y;
  dsu d(n);
  std::vector<std::vector<pair<ll,pll>>> g(n);
  rep(m){
    LL(a,b,t,k);
    --a,--b;
    d.merge(a,b);
  ...
  if(!d.same(x,y))return out(-1);
  vec(ll,dist,n,LINF);
  pq<pll> que;
  dist[x]=0;
  que.push({0,x});
  while(que.size()){
    const auto [cost,fr]=que.top();
    que.pop();
    if(dist[fr]<cost)continue;
    each(to,p,g[fr]){
      const auto [co,le]=p;
      if(chmin(dist[to],dist[fr]+co+rtmod(le-rtmod(dist[fr],le),le))){
        que.push({dist[to],to});
      }
    }
  }
  out(dist[y]<LINF?dist[y]:-1);
