...
signed solve(){//main
  ...
  */
  ...
  const ll szx=x.size();
  if(x.size()==1){
    return out(stoll(x)<=m?1:0);
  }
  ll mx=0;
  each(c,x)chmax(mx,c-'0');

  ll ok=mx,ng=LINF;//半開区間にする
  auto judge=[&](ll mid){
    vector<ll>cur;
    ll num=m;
    ll sz=0;
    while(num){
      cur.push_back('0'+num%mid);
      num/=mid;
      sz++;
    }
    REV(cur);
    //out(cur,sz);
    if(sz<szx)return false;
    if(sz==szx){
      rep(sz){
        if(cur[i]>x[i])return true;
        if(cur[i]<x[i])return false;
      }
    }
    return true;
  };
  while(abs(ok-ng)>1){
    ll mid=(ok+ng)/2;
    //out(mid);
    if(judge(mid))ok=mid;/*最大を求める場合は（mid<=なんかの基準）最小なら向きを反転させれば良い（等号は含む）*/
    else ng=mid;
  }
  out(ok-mx);
