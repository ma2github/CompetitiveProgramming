...
signed main(){
    ...
    while(que.size()){
      ...
      rep(4){
        ...
        if(xi<0||xi>=h||yi<0||yi>=w||s[xi][yi]=='#')continue;
        if(chmin(cost[xi][yi],cost[x][y]))que.push_front(mp(xi,yi));
      }
      rep(xx,-2,3)rep(yy,-2,3){
        ll xi=x+xx,yi=y+yy;
        if(xi<0||xi>=h||yi<0||yi>=w||s[xi][yi]=='#')continue;
        if(chmin(cost[xi][yi],cost[x][y]+1))que.push_back(mp(xi,yi));
      }
    }
    if(cost[dh][dw]==LINF)out(-1);
    else out(cost[dh][dw]);
}
