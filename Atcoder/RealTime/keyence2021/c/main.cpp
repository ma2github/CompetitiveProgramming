...
ll h,w,k;
vv(char,s,5010,5010,'?');
vv(Modint,dp,5010,5010,0);
vv(bool,reach,5010,5010,false);
Modint rec(ll x,ll y){
  if(!x and !y)return 1;
  if(reach[x][y])return dp[x][y];
  reach[x][y]=true;
  Modint res=0;
  if(x and s[x-1][y]!='R'){
    Modint tmp=rec(x-1,y);
    if(s[x-1][y]=='?')tmp/=3,tmp*=2;
    res+=tmp;
  }
  if(y and s[x][y-1]!='D'){
    Modint tmp=rec(x,y-1);
    if(s[x][y-1]=='?')tmp/=3,tmp*=2;
    res+=tmp;
  }
/*  if(x and y+1<w and s[x-1][y+1]=='?')res*=3;
  if(y and x+1<h and s[x+1][y-1]=='?')res*=3;//*/
  return dp[x][y]=res;
}
signed solve(){//main
  /*
  ...
  */
  ...
  Modint ml=modpow(3,h*w-k,MODD);
  ml*=rec(h-1,w-1);
  //Modint mul=s[h-1][w-1]=='?'?3:1;
  out(ml);
  /*
  rep(h){
    rep(j,w)cout<<rec(i,j)<<' ';
    cout<<'\n';
  }
  //*/
