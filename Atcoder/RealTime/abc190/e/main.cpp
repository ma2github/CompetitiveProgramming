...
vv(ll,g,101010);
vec(ll,dist,101010,-1);
vv(ll,cost,20,20,-1);
int k;
vec(ll,c,20);
map<ll,ll>point;
vv(ll,dp,1<<20,20,-1);
ll rec(ll state,int now){
  if(~dp[state][now])return dp[state][now];
  if(state==(1<<now))return dp[state][now]=1;
  ll res=INF;
  ll oldstate=state^(1<<now);
  rep(k){
    if((oldstate>>i)&1){
      //out(i,now,cost[i][now],oldstate,rec(oldstate,i));
      chmin(res,rec(oldstate,i)+cost[i][now]);
    }
  }
...
signed solve(){//main
  ...
  */
  ...
  rep(k-1){
    if(!d.same(c[i],c[i+1]))return out(-1);
  }
  rep(k){
    dist[c[i]]=0;
    queue<ll>que;
    ...
    while(que.size()){
      ...
      each(t,g[x]){
        if(~dist[t])continue;
        dist[t]=dist[x]+1;
        if(point[t] and cost[i][point[t]-1]==-1){
          cost[i][point[t]-1]=dist[t];
          cost[point[t]-1][i]=dist[t];
        }
        que.push(t);
      }
    }
