...
signed solve(){//main
  ...
  */
  ...
  rep(i,n){
    ...
    rep(j,i+1,n){
      ld xj=xy[j].first-xi;
      ld yj=xy[j].second-yi;
      ld dist=xj*xj+yj*yj;
      dist=sqrt(dist);
      dpair.push_back({dist,{i,j}});
    }
    dpair.push_back({yi+100.0,{i,s}});
    dpair.push_back({100.0-yi,{i,t}});
  }
  SORT(dpair);
  each(dist,x,dpair){
    d.merge(x.first,x.second);
    if(d.same(s,t))return out(dist/2.0);
  }
