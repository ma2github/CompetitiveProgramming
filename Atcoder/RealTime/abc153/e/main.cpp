...
vv(ll,dp,10010,1010,LINF);
vec(ll,a,10010,0);
vec(ll,b,10010,0);
ll rec(ll h,ll n){
  if(h<1)return dp[0][n]=0;
  if(n<1)return dp[h][0]=LINF;
  if(dp[h][n]!=LINF)return dp[h][n];
  chmin(dp[h][n],min(rec(h,n-1),rec(h-a[n-1],n)+b[n-1]));
  return dp[h][n];
}
ll rec2(ll h,ll n){
  vec(ll,d,20200,LINF);
  d[0]=0;
  rep(n)rep(j,a[i])chmin(d[j],b[i]);
  rep(n)rep(j,a[i],10001)chmin(d[j],d[j-a[i]]+b[i]);
  return d[h];
}
signed main(){
    /*以下コード*/
