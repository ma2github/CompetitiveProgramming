...
void slv(){//入力と解法を分離させるだけなので，基本的に入力以外何も書かない
  //Input(面倒なときに分離させる)
  cin>>n;
  rep(n)cin>>xy[i].first>>xy[i].second;
  cin>>m;
  rep(m){
    cin>>op[i].first;
  ...
  cin>>q;
  rep(q)cin>>ab[i].first>>ab[i].second;
  solve();//実装本体はこっちに書く（必要に応じて引数を渡す）
}

pair<pll,ll> mns(pair<pll,ll> x){
  return {{-x.first.first,-x.first.second},-x.second};
}

pair<pll,ll> f(pair<pll,ll> x,ll p){
  return {{-x.first.first,-x.first.second},-x.second+2*p};
}

ll F(pair<pll,ll> f,ll x,ll y){
  auto [AB,C]=f;
  auto [A,B]=AB;
  return A*x+B*y+C;
}
vector<pair<pair<pll,ll>,pair<pll,ll>>>dp(200010,{{{0,0},0},{{0,0},0}});
vec(bool,ok,200010,false);

pair<pair<pll,ll>,pair<pll,ll>> rec(ll i){
  if(ok[i])return dp[i];
  ok[i]=true;
  if(!i)return dp[i]={{{1,0},0},{{0,1},0}};
  auto [t,p]=op[i-1];
  auto [x,y]=rec(i-1);
  if(t==1){
    return dp[i]={y,mns(x)};
