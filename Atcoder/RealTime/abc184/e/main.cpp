...
signed solve(){//main
  /*
  idea:
  S->wp+wp->wp+G->wp(a<=wp<=z)の最短距離 or S->Gへの最短距離が答え
  BFSでいけそう
  存在しないなら-1
  */
  INT(h,w);
  VEC(string,s,h);
  std::vector<std::vector<pii>> fr(26);
  deque<pii>que;
  vv(int,dist,h,w,-1);
  vec(bool,fin,26,false);
  ll ans=LINF;
  rep(i,h)rep(j,w){
    if(islower(s[i][j]))fr[s[i][j]-'a'].push_back({(int)i,(int)j});
    if(s[i][j]=='S')dist[i][j]=0,que.push_back({(int)i,(int)j});
  }
  while(que.size()){
    int x=que.front().first,y=que.front().second;
    que.pop_front();
    rep(i,4){
      int nx=x+(int)dx[i],ny=y+(int)dy[i];
      if(nx<0 or nx>h-1 or ny<0 or ny>w-1)continue;
      char now=s[nx][ny];
      if(now=='#')continue;
      if(~dist[nx][ny]){continue;}
      dist[nx][ny]=dist[x][y]+1;
      if(now=='G')ans=dist[nx][ny];
      que.push_back({nx,ny});
    }
    if(islower(s[x][y])){
      if(fin[s[x][y]-'a'])continue;
      each(idx,fr[s[x][y]-'a']){
