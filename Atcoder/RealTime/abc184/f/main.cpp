...
signed solve(){//main
  ...
  */
  ...
  vec(ll,tot,1<<(n/2),0);
  rep(1<<(n/2)){
    rep(j,n/2)if((i>>j)&1)tot[i]+=a[2*j];
  }
  sort(all(tot));
  rep(1<<(n/2)){
    ll cur=0;
    rep(j,n/2)if((i>>j)&1)cur+=a[2*j+1];
    rep(_,n%2+1){
      ll ok=0,ng=1<<(n/2);//半開区間にする
      while(abs(ok-ng)>1){
        ll mid=(ok+ng)/2;
        if(tot[mid]+cur<=t)ok=mid;/*最大を求める場合は（mid<=なんかの基準）最小なら向きを反転させれば良い（等号は含む）*/
        else ng=mid;
      }
      if(cur+tot[ok]<=t)chmax(ans,cur+tot[ok]);
      cur+=a[n-1];
    }
  }
  out(ans);
