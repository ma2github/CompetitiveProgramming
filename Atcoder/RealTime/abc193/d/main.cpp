...
signed solve(){//main
  ...
  */
  ...
  STR(t);
  ll ts=55,as=55;
  map<ll,ll>memo1,memo2;
  each(x,s){
    if(x=='#')continue;
    ts+=(x-'0')*intpow(10,memo1[x-'0'])*9;
    memo1[x-'0']++;
  }
  each(x,t){
    if(x=='#')continue;
    as+=(x-'0')*intpow(10,memo2[x-'0'])*9;
    memo2[x-'0']++;
  }
  ld ans=0.0;
  rep(i,1,10){
    ll ci=memo1[i]+memo2[i]+1;
    if(ci>k)continue;
    rep(j,1,10){
      ll cj=memo1[j]+memo2[j]+1;
      if(i==j)cj++;
      if(cj>k)continue;
      ll ats=(i)*intpow(10,memo1[i])*9;
      ll aas=(j)*intpow(10,memo2[j])*9;
      if(ts+ats>as+aas){
        ld tmp=0.0;
        if(i==j){
          ll c=k-memo1[i]-memo2[i];
          tmp=(ld)c/(ld)(9*k-8);
          c--;
          tmp*=(ld)c/(ld)(9*k-9);
        }
