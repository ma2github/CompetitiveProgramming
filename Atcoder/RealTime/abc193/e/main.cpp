...
signed solve(){//main
  /*
  idea:
  x=i(mod A),x=p(mod B)を満たすxは(i=p mod(gcd(A,B)))を満たす時かつその時に限り存在し，以下のように表せる
  x=(i-p)/g*B*n+p
  ここでn=1/B'(mod A')(A'=A/g,B'=B/gとする)
  nの構成については，A'm+B'n=1の不定方程式を考えたときのmod A'を考えてみると良い
  ref:https://qiita.com/drken/items/ae02240cd1f8edfc86fd
  */
  LL(x,y,p,q);
  ll ans=LINF;
  const ll A=2*(x+y),B=p+q;
  const ll G=gcd(A,B);
  const ll uA=A/G,uB=B/G;
  const ll mB=modinv(uA,uB),mA=modinv(uB,uA);
  rep(i,x-p,x+y-p){
    if(i%G)continue;
    ll cur=(i/G)*mA;
    mod(cur,uA);
