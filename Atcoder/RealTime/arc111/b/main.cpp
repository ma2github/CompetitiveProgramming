...
signed solve(){//main
  ...
  */
  INT(n);
  dsu d(400010);
  set<ll>cy,tr,cyc,mem;
  VEC(ll,ab0,2*n);
  unordered_map<ll,ll>p=press(ab0);
  vec(pll,ab,n);
  rep(n){
    ab[i]={ab0[2*i],ab0[2*i+1]};
  }
  each(x,ab){
    if(d.same(x.first,x.second))cy.insert(d.leader(x.first));
    d.merge(x.first,x.second);
    mem.insert(x.first);
    mem.insert(x.second);
  }
  each(x,cy)cyc.insert(d.leader(x));
  each(x,mem){
    if(!cyc.count(d.leader(x)))tr.insert(d.leader(x));
  }
  ll ans=0;
  each(x,cyc)ans+=d.size(x);
  each(x,tr)ans+=d.size(x)-1;
  out(ans);
