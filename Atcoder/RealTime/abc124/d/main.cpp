...
signed solve(){//main
  /*
  idea:1....1，0...0，1...1,...と分けたときの2k+1個分の区間和の最大を取る
  区間が2k+1も無ければ全部行ける
  */
  LL(n,k);
  STR(s);
  ll cnt2=0,cnt3=0;
  ll index=0;
  while(s[index]=='1')cnt3++,index++;
  std::vector<ll> sm;
  sm.push_back(cnt3);
  ll i=index;
  while(i<n){
    cnt2=0,cnt3=0;
    while(i<n and s[i]=='0')cnt2++,i++;
    while(i<n and s[i]=='1')cnt3++,i++;
    sm.push_back(cnt2);
    sm.push_back(cnt3);
  }
  if(sm.size()<=2*k+1)return out(n);
  vec(ll,smsm,sm.size()+1,0);
  rep(sm.size()){
    smsm[i+1]=smsm[i]+sm[i];
  }
  ll ans=0;
  rep(i,0,smsm.size()-2*k-1,2){
    chmax(ans,smsm[i+2*k+1]-smsm[i]);
  }
  out(ans);
