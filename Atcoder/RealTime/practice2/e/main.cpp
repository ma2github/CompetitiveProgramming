...
signed solve(){//main
  ...
  */
  ...
  int s=2*n,t=2*n+1;//始点，終点
  g.add_edge(s,t,n*k,INF);//頂点を流れないフローの行き先（最終的にコストから外れるので，INF-0とする）
  rep(n)g.add_edge(s,i,k,0),g.add_edge(n+i,t,k,0);//行or列あたりkまでしか流せない（0は便宜上，これ以上コストを入れようとすると頭壊れる）
  rep(i,n)rep(j,n){
    g.add_edge(i,n+j,1,INF-a[i][j]);//行→列へのフローのコストと捉える
  }
  auto fl=g.flow(s,t,n*k);//maxでn行*1行あたりk列流すので
  out(n*k*INF-fl.second);
  vec(string,ans,n,string(n,'.'));
  auto ed=g.edges();
  each(x,ed){
    if(x.from==s or x.to==t or x.flow==0)continue;
    int i=x.from,j=x.to-n;
    ans[i][j]='X';
  }
  each(x,ans)out(x);
