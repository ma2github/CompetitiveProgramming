...
signed solve(){//main
  ...
  */
  ...
  rep(i,n)rep(j,i+1,n){
    ...
    int xj=xy[j].first,yj=xy[j].second;
    if(abs(xi-xj)<d)ts.add_clause(i,false,j,false);
    if(abs(xi-yj)<d)ts.add_clause(i,false,j,true);
    if(abs(yi-xj)<d)ts.add_clause(i,true,j,false);
    if(abs(yi-yj)<d)ts.add_clause(i,true,j,true);
  }
  Yes(ts.satisfiable());
  if(ts.satisfiable()){
    auto ans=ts.answer();
    rep(n){
      out(ans[i]?xy[i].first:xy[i].second);
    }
  }
