...
signed solve(){//main
  ...
  */
  ...
  r=rd(4);
  ll cur=x-r;
  cur+=(10000-rtmod(cur,10000))%10000;
  //out(x,y,r,cur);
  ll ans=0;
  auto bs=[&](ll now,ll ngini){
    ll ok=y,ng=ngini;//半開区間にする
    while(abs(ok-ng)>1){
      ll mid=(ok+ng)/2;
      if((now-x)*(now-x)+(mid-y)*(mid-y)<=r*r)ok=mid;/*最大を求める場合は（mid<=なんかの基準）最小なら向きを反転させれば良い（等号は含む）*/
      else ng=mid;
    }
    return ok;
  };
  while(cur<=x+r){
    ll p,m;
    //if(!cur)
    //out((ll)bs(cur,y+r+1)/10000,(ll)ce(bs(cur,y-r-1)));
    p=bs(cur,y+r+1);
    p-=rtmod(p,10000);
    p/=10000;
    m=bs(cur,y-r-1);
    m+=(10000-rtmod(m,10000))%10000;
    m/=10000;
    if(p>=m)ans+=p-m+1;
    cur+=10000;
  }
