...
signed solve(){//main
  ...
  */
  LL(n,m);
  mcf_graph<int,int>g(2*n);
  while(m--){
    INT(a,b,c);
    --a,--b;
    g.add_edge(a,b+n,1,c);
    g.add_edge(a+n,b+n,1,c);
  }
  auto tmp=g;
  rep(i,n){
    auto [flow,cost]=g.flow(i,i+n,1);
    out(flow?cost:-1);
    g=tmp;
    //auto es=g.edges();
/*
    if(flow){
      //auto es=g.edges();
      ll ans=0;
      each(e,es)if(e.flow)ans+=e.cost;
      out(ans);
    }else{
      out(-1);
      each(e,es){
        out(e.from,e.to,e.flow,e.cost);
      }
    }//*/
    /*
    auto ed=g.edges();
    each(x,ed){
      out(x.from,x.to,x.flow,x.cost);
    }//*/
  }
