...
ll dfs(ll n,std::vector<std::vector<ll>> &b){
  if(!b[n].size())return 1;
  ll mn=LINF,mx=0;
  each(i,b[n])chmin(mn,dfs(i,b)),chmax(mx,dfs(i,b));
  return mx+mn+1;
}
signed main(){
    /*以下コード*/
    LL(n);
    vec(std::vector<ll>,buka,n);
    rep(n-1){
      LL(bi);
      buka[bi-1].push_back(i+1);
    }
    out(dfs(0,buka));
    /*
    ...
    while(que.size()){
      ...
      if(!cnt)chmin(mn,dist[v]),chmax(mx,dist[v]);
    }*/
}
