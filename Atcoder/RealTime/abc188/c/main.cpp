...
signed solve(){//main
  ...
  */
  ...
  VEC(ll,a,1<<n);
  vec(pll,tor,1<<n);
  rep(1<<n)tor[i]={a[i],i+1};
  rep(n-1){
    rep(j,1<<(n-1-i)){
      ll win=max(tor[2*j].first,tor[2*j+1].first);
      if(tor[2*j].first==win){
        tor[j]={win,tor[2*j].second};
      }else{
        tor[j]={win,tor[2*j+1].second};
      }
    }
  }
  ll ans=tor[1].second;
  if(tor[0].first<tor[1].first)ans=tor[0].second;
  out(ans);
