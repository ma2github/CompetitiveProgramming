...
signed solve(){//main
  ...
  */
  ...
  std::vector<int> ans;
  std::vector<ll> v;
  rep(n)v.push_back(i+1);
  vec(int,idx,n+1);
  rep(n)idx[a[i]]=i+1;
  rrep(n){
    while(idx[i+1]<i+1){
      int id=idx[i+1];
      if(used[id] or sz>n-1)return out(-1);
      ans.push_back(id);
      idx[a[id]]=id;
      swap(a[id-1],a[id]);
      idx[i+1]++;
...
/*
  std::vector<int> sw;
  map<std::vector<int>,ll> memo;
  int n=5;
  rep(n-1)sw.push_back(i);
  do{
    std::vector<int> v;
