...
signed main(){
    ...
    rep(n){
      ...
      memob[bi]++;
      deq.push_back(bi);
      if(memob[bi]==n-memoa[bi])ari=bi;
      if(memob[bi]>n-memoa[bi])ok=false;
    }
    std::vector<int> v;
    if(ok){
      rep(n){
        ...
        if(ari){
          ...
          if(a[i]!=ari)d=ari;
            ...
            each(i,memob){if(!i.second)continue;d=i.first,memob[i.first]--;break;}
          }
        }
        else{if(deq.back()!=a[i]){d=deq.back();deq.pop_back();}
        elif(deq.front()!=a[i]){d=deq.front();deq.pop_front();}
        else ok=false;}
        v.push_back(d);
      }
    }
