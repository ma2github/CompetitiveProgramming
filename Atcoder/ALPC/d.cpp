...
signed solve(){//main
  ...
  */
  ...
  rep(i,n)rep(j,m){
    if(map[i][j]=='#')continue;
    int v=m*i+j;
    if((i+j)%2)graph.add_edge(s,v,1);
    else graph.add_edge(v,t,1);
  }
  rep(i,n)rep(j,m){
    if(map[i][j]=='#')continue;
    if((i+j)%2==0)continue;
    rep(k,4){
      ...
      if(ni>n-1 or nj>m-1 or ni<0 or nj<0)continue;
      if(map[ni][nj]=='#')continue;
      int u=i*m+j,v=ni*m+nj;
  ...
  each(x,edges){
    ...
    int u=min(x.from,x.to),v=max(x.from,x.to);
    int ui=u/m,uj=u%m;
    int vi=v/m,vj=v%m;
    if(ui==vi){
      map[ui][uj]='>';
