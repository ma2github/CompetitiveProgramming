...
ll rec(ll l,ll r,ll s){
  ...
  if(l==r){
    if(!s)return dp[l][r]=a[l];
    return dp[l][r]=0;
  }
  if(!s)return dp[l][r]=max(rec((l+1)%n0,r,1)+a[l],rec(l,(r-1+n0)%n0,1)+a[r]);
  else{
    if(a[l]<a[r])return dp[l][r]=rec(l,(r-1+n0)%n0,0);
    else return dp[l][r]=rec((l+1)%n0,r,0);
  }
}
signed main(){
    /*以下コード*/
