...
vv(ll,dist,16,16,LINF);
vv(ll,timest,16,16,-1);
vv(pll,dp,1<<16,16,{-1,-1});
pll rec(ll bit,ll t){
  if(~dp[bit][t].first)return dp[bit][t];
  if(bit==(1<<t)&&!t)return dp[bit][t]={0,1};
  pll res={LINF,0};
  ll pbit=bit&~(1<<t);
  rep(16){
    if(!((pbit>>i)&1))continue;
    pll pr={rec(pbit,i).first+dist[i][t],rec(pbit,i).second};
    if(pr.first>timest[i][t])continue;
    if(res.first>pr.first)res=pr;
    elif(res.first==pr.first)res.second+=pr.second;
  }
  return dp[bit][t]=res;
}
pll tle(ll n){
  ...
  map<ll,ll> memo;
  std::vector<int> v;
  rep(n-1)v.push_back(i+1);
  do{
    ll di=0;
    ll pl=0;
    bool f=true;
    rep(n-1){di+=dist[pl][v[i]];if(di>timest[pl][v[i]])f=false;pl=v[i];}
    if(di+dist[pl][0]>timest[pl][0])f=false;
    if(f)chmin(ans,di+dist[pl][0]),memo[di+dist[pl][0]]++;
  }while(next_permutation(all(v)));
  pll answer={ans,memo[ans]};
  return answer;
}
signed main(){
    /*以下コード*/
    ...
    pll ans={LINF,0};
    rep(n){
      pll pr={rec((1<<n)-1,i).first+dist[i][0],rec((1<<n)-1,i).second};
      if(pr.first<=timest[i][0]){
      if(ans.first>pr.first)ans=pr;
      elif(ans.first==pr.first)ans.second+=pr.second;
      }
    }
    if(ans.first==LINF)return POSSIBLE(0);
    out(ans);
}
