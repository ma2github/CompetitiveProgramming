...
std::vector<std::vector<ll>> m(101,std::vector<ll>(2,0));
std::vector<std::vector<ll>> dp(101,std::vector<ll> (101,-1));
ll rec(ll l,ll r){
  if(r-l<2)return dp[l][r]=0;
  if(~dp[l][r])return dp[l][r];
  ll res=LINF;
  rep(mid,l+1,r)chmin(res,rec(l,mid)+rec(mid,r)+m[l][0]*m[mid][0]*m[r-1][1]);
  return dp[l][r]=res;
}
signed main(){
    /*以下コード*/
