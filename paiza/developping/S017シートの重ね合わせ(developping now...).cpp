...
map<char, int> memo,memo1;
deque<tuple<char, int,int>> memory;
signed main(){
    /*以下コード*/
    int max=0;
    char lastc;
    ...
    each(i,c)each(j,i){
      if(j!='.'){
      memo[j]++;
      if(memo[j]==1)memo1[j]=locate;
      if(memo[j]==m*m)lastc=j;
    }
      locate++;
  }
    each(x,memo)if(max<x.second){max=x.second;lastc=x.first;}
    //while(colornum<s){
    locate=memo1[lastc];
    int locx=int(locate/n);
    int locy=locate%n;
    memory.push_front(make_tuple(lastc,locx+1,locy+1));
    /*Todo
    この部分に一番上だった色を取り外した時の色の変化の仕方を記述する
    これを色の回数分繰り返すことで全てのますを.（何もない状態）にできる
    */
  //}
  while(!memory.empty()){//output
    tuple<char,int,int> t=memory.front();
    lastc=get<0>(t);
    locx=get<1>(t);
    locy=get<2>(t);
    cout<<lastc<<' '<<locx<<' '<<locy<<endl;
    memory.pop_front();
  }
}
